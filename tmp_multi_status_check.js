      ...
      document.addEventListener('DOMContentLoaded', () => {
        ...
        console.log('[状态栏 全局] DOMContentLoaded，检测到状态栏面板数量:', panels.length);

        panels.forEach(panel => {
          const panelId = panel.id || '未知';
          const debugPrefix = `[状态栏 ${panelId}]`;
          let debugHooksInstalled = false;
          function debugLog(message, details) {
            if (details === undefined) console.log(`${debugPrefix} ${message}`);
            else console.log(`${debugPrefix} ${message}`, details);
          }
          function debugError(message, error, details) {
            if (details === undefined) console.error(`${debugPrefix} ${message}`, error);
            else console.error(`${debugPrefix} ${message}`, error, details);
          }
          function installDebugHooks() {
            if (debugHooksInstalled) return;
            if (typeof window === 'undefined' || typeof window.addEventListener !== 'function') return;
            window.addEventListener('error', event => {
              debugError('捕获到全局 error 事件。', event.error || event.message, {
            ...
            window.addEventListener('unhandledrejection', event => {
              debugError('捕获到未处理的 Promise 拒绝。', event.reason);
            });
            debugHooksInstalled = true;
            debugLog('调试钩子已安装。');
          }
          function displayError(message, details) {
            const detailText = details === undefined ? '' : details;
            console.error(`${debugPrefix} ${message}`, detailText);
            panel.innerHTML = `<div style="color: var(--red-color); padding: 20px; font-family: monospace; white-space: pre-wrap;"><h3>状态栏错误</h3><p>${message}</p><details><summary>详细信息</summary><p>${detailText}</p></details></div>`;
          }
          ...
          try {
            ...
            debugLog('rawData JSON 解析成功。', {
              rawDataLength: rawData.length,
              name: jsonData && jsonData.Name !== undefined ? jsonData.Name : undefined,
              identity: jsonData && jsonData.Status && jsonData.Status.Identity !== undefined ? jsonData.Status.Identity : undefined,
            });

            function safeGet(obj, path, defaultValue) {
              const fallbackValue = arguments.length >= 3 ? defaultValue : undefined;
              const keys = Array.isArray(path) ? path : path.split('.');
              let result = obj;
              for (const key of keys) {
                if (result === undefined || result === null) return fallbackValue;
                result = result[key];
                if (result === undefined) return fallbackValue;
              }
              return result;
            }
            ...
            async function fetchData() {
              try {
                ...
                debugLog('已从全局管理器获取数据。', {
                  imageKeywordCount: Object.keys(imageIndex).length,
                  synonymCount: Object.keys(synonymMap).length,
                  fallbackAvatarCount: fallbackAvatars.length,
                });
                return true;
              } catch (error) {
                debugError('从全局管理器获取数据失败。', error);
            ...
            function parseStat(statValue) {
              ...
              if (rangeMatch) {
                return {
                  ...
                  from: rangeMatch[1],
                  to: rangeMatch[2],
                  reason: rangeMatch[3] === undefined || rangeMatch[3] === null ? '' : rangeMatch[3].trim(),
                };
              }
              ...
              if (singleMatch) {
                return {
                  mode: 'single',
                  value: singleMatch[1],
                  reason: singleMatch[2] === undefined || singleMatch[2] === null ? '' : singleMatch[2].trim(),
                };
              }
              return null;
            }
              ...
              for (const key in data) {
                if (
                  ...
                  for (const subKey in data[key]) {
                    ...
                    const valueHtml = `<span class="details-value">${escapeHtml(value)}</span>`;
                    contentsHtml += `
            ...
            function updatePanel(data) {
              debugLog('开始渲染面板。', { name: data && data.Name !== undefined ? data.Name : undefined, identity: safeGet(data, 'Status.Identity', '') });
              // 1. 设置只包含静态部分的HTML框架
              panel.innerHTML = `
                        <div class="header">
                            <div class="portrait">
              ...
              debugLog('面板渲染完成。', {
                tabCount: panel.querySelectorAll('.tab-button').length,
            ...
            async function main() {
              ...
              if (dataLoaded) {
                ...
                console.warn(`${debugPrefix} 无法加载图片索引，图片相关功能将不可用。`);
              }
              setupEventListeners();
              debugLog('main() 执行完成。');
            }
            ...
            main().catch(error => {
              displayError(
                '脚本执行失败。这通常是由于状态栏初始化或渲染阶段发生错误导致的。',
