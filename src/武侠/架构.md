...
└── utils/               # 工具函数目录
    ...
    ├── messageActions.ts      # 重新生成、ERA强制同步等消息动作
...
sequenceDiagram
    ...
    Extra->>ERA: era:forceSync({mode:'latest'})
...
- 最近正文只取最新 5 条有效 assistant 正文，并在发送给额外模型前剥离旧的 `<VariableThink>`、`<VariableInsert>`、`<VariableEdit>`、`<VariableDelete>` 块。
- `输出提示词` 不把原始模板发给外部模型；先在酒馆前端环境内用 `EjsTemplate.prepareContext(..., assistantMessageId)`、`EjsTemplate.evaltemplate`、`substitudeMacros` 和 Tavern regex 渲染，失败时退回前端构造的变量快照。
- 变量块只接受 `<VariableThink>`、`<VariableInsert>`、`<VariableEdit>`、`<VariableDelete>`；Insert/Edit/Delete 内必须是严格 JSON 对象。
...
- 重新生成先通过 `era:forceSync({mode:'rollbackTo', message_id:userMessageId})` 回滚到上一条 user 楼层，再生成新 swipe、同步 latest，最后复用同一套额外变量更新流程。
...
额外模型配置统一挂在 `DisplaySettings.summarySettings`，由 `settingsManager.ts` 负责 localStorage 迁移与默认值补全。设置页对外显示为“额外模型”，内部按 `API / 自动总结 / 额外变量` 分组；其中 API 配置被自动总结和额外变量更新复用，不拆成两套凭据。
...
| `messageActions.ts` | 封装 `era:forceSync` 等待、最新 assistant swipe 重新生成与 rollback/latest 同步 |

`requestConfiguredText` 的两条后端路径：

1. `apiMode: 'preset'`：调用酒馆助手 `generateRaw()`，只传入单条 user prompt，并用 overrides 清空世界书、角色描述、示例对话和 chat history；额外变量更新还会启用 `skipWorldInfoAndAuthorNote`，在生成事件中补写 `skipWIAN`，避免世界书和作者注再次进入请求。
2. `apiMode: 'custom'` 且 `source: 'custom'`：直接请求酒馆后端 `/api/backends/chat-completions/generate`，构造 OpenAI 兼容 messages，请求头通过 `SillyTavern.getRequestHeaders()` 获取。

自动总结写回策略：

- `checkSummaryTrigger` 统计玩家和 NPC 的 `人物经历` 条目数，支持对象和字符串两种格式，并过滤 `$` 开头模板字段。
- 触发条件包括待处理角色数阈值、总条目数阈值、单角色条目阈值。
- `executeAllPendingSummaries` 顺序处理待总结角色，解析 `<summary>` 内容后转为新的 `人物经历` 对象，通过 `era:updateByObject` 写回。

#### 3.6.6. 设置面板运行时控制台

`SettingsPanel.tsx` 已不只是外观设置组件，而是前端的运行时控制台。它按 `appearance / regex / summary / variables / advance / debug` 六个页签组织入口；其中 `summary` 对应额外模型配置，`variables` 和 `advance` 会直接读写聊天变量或复用发送流程。
...
自动推进不是独立生成通道：`handleAutoAdvanceTurn` 内部调用普通 `handleSendMessage`，因此它继承正文生成、额外变量更新、ERA 同步和错误处理的同一套流程。
