...
实际参与 diff 的快照必须来自同一种表示。当前 canonical source 是 `getVariables({ type: 'chat' }).stat_data`；`era:writeDone` 的 `stat/statWithoutMeta` 在广播前已被 ERA 反转义，所以只用于归因元数据，不能直接参与 hash 或 diff。

若 `era:writeDone` 到达时聊天态快照还没推进，追踪器只重读聊天态 `stat_data`，按 `40ms` 间隔、最多 `8` 次短轮询补读。
