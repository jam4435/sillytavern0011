...
sequenceDiagram
    ...
    Handler->>Shell: 解锁并等待处理完成
    Handler->>Shell: 同步到最新宿主楼层
```
...
12. `finally` 中释放额外变量预留，等待 `wuxia:turn-lifecycle(finish)` 解锁处理完成，再请求同步最新宿主楼层。
...
7. 回合锁 ACK 必须先于真实 user 楼层；解锁完成必须先于宿主楼层同步。
