...
重新生成由 `handleRegenerateLastAssistant()` 和 `utils/messageActions.ts` 处理。底栏的循环箭头仍是一键重新生成；其窄展开区只提供“修改上一轮输入后重新生成”。进入该模式后，指令队列与普通重生按钮收起，Enter 和右侧唯一主按钮都调用带 `replacementUserInput` 的重新生成链，不再允许把修改后的旧输入误发成新 user 楼层。
