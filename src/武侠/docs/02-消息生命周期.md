...
sequenceDiagram
    ...
    Handler->>Handler: 构造动态地图地点约束
    Handler->>Tavern: generate(should_stream:true, injects:地点约束)
...
5. `locationContext.ts` 根据 `stat_data.user数据.所在位置` 注入当前二级地点的三级地点和坐标最近的相邻二级地点白名单。
6. `generate({ should_stream: true, injects })` 生成完整 raw 回复。
...
4. 回滚完成后按当前 `stat_data` 重建动态地图地点约束，再调用 `generate()` 生成新正文。
