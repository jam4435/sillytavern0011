...
5. `locationContext.ts` 根据 `stat_data.user数据.所在位置` 刷新聊天变量顶层的 `地图上下文`，不直接注入提示词。
6. `generate({ should_stream: true })` 生成完整 raw 回复；世界书是否读取地图上下文、放在何处由世界书条目自行决定。
7. 解析 `<option>`，创建真实 assistant 楼层，并刷新 React 显示。
8. `extra` 模式继续把变量块写入聊天数据，保持当前楼层 shell 不重绑，并等待 ERA 同步。
9. `finally` 中释放额外变量预留并结束加载状态。
...
4. 回滚完成后按当前 `stat_data` 刷新聊天变量 `地图上下文`，再调用 `generate()` 生成新正文。
...
发送和重新生成只负责在生成前刷新聊天变量顶层的 `地图上下文`。该对象包含：
...
```ejs
<% const mapContext = getVariables({ type: 'chat' }).地图上下文; %>
相邻三级地点：<%= mapContext ? JSON.stringify(mapContext.相邻三级地点) : '[]' %>
相邻二级地点：<%= mapContext ? JSON.stringify(mapContext.相邻二级地点) : '[]' %>
```
