...
只有模板实际包含 `{{locationContext}}` 时才会插入动态地点约束，前端不会在模板末尾强行追加；可在设置页移动或删除。地点使用 `一级/二级/三级[/四级]`：前三段必须逐字复用动态白名单中的严格活动区，第四段可按正文维护且不参加白名单匹配。同一活动区内可新增、修改或移除第四级；跨活动区必须命中普通移动、事件目标或地图指定路径的前三段。无法从地点表解析当前位置时，本轮不得修改任何 `所在位置`。
