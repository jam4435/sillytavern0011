...
事件系统统一使用 **12 月×30 天＝360 天/年** 的简化历法。模块外的 `月`、`日` 均为 1 基下标，合法范围为
`月=1–12`、`日=1–30`；时和分分别使用 `0–23`、`0–59`。统一换算由 `src/shared/wuxiaCalendar.js` 提供：
...
时间比较、事件时长、提前启动后的实际结束时间、相对事件重排、manifest 索引、checkpoint 选择和前端剩余天数都必须复用这套换算，禁止自行使用
`365 天/年`、`Date.UTC` 或 `月 × 30 + 日`。例如：
...
修改历法实现或事件时间后必须重新运行
`pnpm generate:events`，使 manifest、分片与 checkpoint 与运行时一致。修复不会自动改写已有聊天里已经归档的错误日期或错误完成状态；这类聊天需要从未污染节点恢复、定向清理状态，或新建聊天复测。
...
`scripts/audit-wuxia-event-durations.mjs` 与 `scripts/fix-wuxia-event-durations.mjs`
也使用同一共享历法；离线审计结果与游戏运行时不得采用不同的公历或年长口径。
...
- 进入提前十天窗口的所有可发现事件另写入前端只读
  `前端变量.可发现事件`；它不进入“附近传闻”提示词，只向江湖事簿公开事件引子、开始时间与完整地点，供玩家主动赶赴
